    ...
    public class MarsModel : PageModel
    {
        public async Task<List<Photos>> GetPhotos()
        {
            ...
            try
            {
                ...
                {
                    Photos photo = new();
                    photo.Sol = reader.GetString("Sol");
                    photo.Camera = (IDictionary<string, string>)JsonConvert.DeserializeObject(reader.GetValue("Camera").ToString());
                    photo.Img_src = reader.GetString("Img_src");
                    photo.Earth_date = reader.GetString("Earth_date");
                    photo.Rover = (IDictionary<string, string>)JsonConvert.DeserializeObject(reader.GetValue("Rover").ToString());
                    Photos.Add(photo);
                }
            }catch (Exception ex) { }
            return Photos;
